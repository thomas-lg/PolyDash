...
public class Reader {
	...
	/**
	 * Permet de transcrire un fichier correctement formaté en un dammier de
	 * 20*20 ( le plateau de jeu) Retourne un message d'erreur si le fichier
		...
		while ((line = readwithBuffer.readLine()) != null && index < Var.NBROW) {
			try {
				...
				JOptionPane.showMessageDialog(
						null,
						"Is not the right format for"
								+ this.level.getAbsolutePath());
	...
	/**
	 * Permet d'ouvrir un fichier, retourne un message à l'utilisateur si le
	 * fichier est introuvable
	 * 
	 * @param f
	 *            correspond au fichier à ouvrir
	...
	private BufferedReader openFile(File f) {
		try {
			...
			JOptionPane.showMessageDialog(null,
					"File not found in .../PolyDash/level/");
			e.printStackTrace();
			return null;
		}
	}
	...
	private Block getInstanceBlock(char c) throws UndefineCharBlockException {
		switch (c) {
		case '_':
			return new BlockEmpty(Polydash.res.getTexture("empty"));
		case '#':
			return new BlockFix(Polydash.res.getTexture("fix_bloc"));
		case 'O':
			return new BlockMovable(Polydash.res.getTexture("mobile_bloc"));
		case 'G':
			return new BlockGem(Polydash.res.getTexture("ruby"));
		case 'P':
			return new Character(Polydash.res.getTexture("miner"));
		default:
			throw new UndefineCharBlockException();
		}
	}
